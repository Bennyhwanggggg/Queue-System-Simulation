...
setup_status = zeros(m, 1);
delayed_off_status = zeros(m, 1);
...
%while num_customer_served < time_end %|| any(server_status)
while condition
    ...
    %
    % We use next_event_type = 1 for arrival and 0 for departure May need
    % to change this
    %
...
%     display('New Event Here------------')
%     display(next_event_type)
    % update master clock
    master_clock = next_event_time;
%     display(master_clock)
...
%                 display('No server in delayed off but idle server found.');
        ...
        % started counting down) and change it back to busy
            ...
            %             server_status(first_delayoff_server) = 3; % set to busy
%             next_delayed_off_time(first_delayoff_server) = Inf; % server no longer in delayed off so change timer back to inf
%             next_departure_time(first_delayoff_server) = next_arrival_time + service_time_next_arrival;
%             % arrival time of next departure tells us the arrival time of
%             % the job so we can use it for response time calculation after
%             arrival_time_next_departure(first_delayoff_server) = next_arrival_time;
        end
...
%             display('A server has finished setup and there is a marked job in the queue to take');
    ...
    elseif (next_event_type == 0) % A job has finished in a server and will now depart
        ...
        if queue_length
%             display('A job has finished but marked job detected in queue so this server will take another job')
...
%             display('A job has finished and no job in queue. Server go to delay off')
